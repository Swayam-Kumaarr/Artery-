// Warli art is a tribal style from Maharashtra — geometric stick figures,
// concentric circles, triangles for trees, all in earthy ink lines.
...
export function WarliArt({
  ...
  // Each variant is a unique hand-drawn-style Warli SVG scene
  if (variant === "corner-tl") {
    // Top-left: the classic Warli "chauk" (central circle) with dancing figures
    return (
      <svg
        viewBox="0 0 300 300"
        ...
        aria-hidden
      >
        {/* Central circle (sun / life source) */}
        <circle cx="80" cy="80" r="32" />
        <circle cx="80" cy="80" r="22" />
        <circle cx="80" cy="80" r="10" />
        {/* Radiating lines from circle */}
        {[0,30,60,90,120,150,180,210,240,270,300,330].map((deg) => {
          const rad = (deg * Math.PI) / 180;
          return (
            <line
              key={deg}
              x1={r2(80 + 34 * Math.cos(rad))}
              y1={r2(80 + 34 * Math.sin(rad))}
              x2={r2(80 + 44 * Math.cos(rad))}
              y2={r2(80 + 44 * Math.sin(rad))}
            />
          );
        })}
        {/* Dancing stick figures arranged in a circle */}
        {[0,72,144,216,288].map((deg, i) => {
          ...
          return (
            <g key={i} transform={`translate(${cx},${cy}) rotate(${deg + 90})`}>
              <circle cx="0" cy="-10" r="3.5" /> {/* head */}
              <line x1="0" y1="-6.5" x2="0" y2="4" /> {/* body */}
              <line x1="-6" y1="-2" x2="6" y2="-2" /> {/* arms */}
              <line x1="0" y1="4" x2="-5" y2="11" /> {/* left leg */}
              <line x1="0" y1="4" x2="5" y2="11" />  {/* right leg */}
            </g>
          );
        })}
        {/* Warli tree (triangle + trunk) */}
  ...
  if (variant === "corner-br") {
    // Bottom-right: harvest scene — wheat, figures carrying pots
    return (
      <svg
        viewBox="0 0 280 280"
        className={`pointer-events-none select-none ${className}`}
      ...
      >
        {/* Ground line */}
        <line x1="0" y1="230" x2="280" y2="230" />
        {/* Wheat stalks */}
        ...
        {/* Moon crescent */}
        <path d="M245 30 Q255 50 245 70 Q270 50 245 30Z" />
  ...
  if (variant === "band") {
    // Horizontal Warli frieze — figures dancing in a line
    return (
      <svg
        viewBox="0 0 800 80"
        className={`pointer-events-none select-none ${className}`}
      ...
      >
        ...
        <line x1="0" y1="8" x2="800" y2="8" />
        <line x1="0" y1="72" x2="800" y2="72" />
        {/* Repeating dancing figures + elements */}
        {Array.from({ length: 16 }, (_, i) => {
          ...
          return (
            <g key={i}>
              ...
              <line x1={x} y1="50" x2={x + 7} y2="62" />
              {/* Tiny triangle tree between figures */}
              {i % 3 === 1 && (
                <>
  ...
  if (variant === "side-right") {
    // Tall vertical strip — a narrative scene (boat, birds, figures)
    return (
      <svg
        viewBox="0 0 120 500"
        className={`pointer-events-none select-none ${className}`}
  ...
  // "scatter" — small scattered motifs for section fills
  return (
    <svg
      viewBox="0 0 400 200"
      className={`pointer-events-none select-none ${className}`}
